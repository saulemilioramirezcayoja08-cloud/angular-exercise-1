...
import { OrderPrintData } from './models/order-print-data.model';

@Injectable({
  providedIn: 'root'
})
export class OrderPrintService {
  private printDataSubject = new BehaviorSubject<OrderPrintData | null>(null);

  setPrintData(data: OrderPrintData): void {
    this.printDataSubject.next(data);
  }

  getPrintData(): Observable<OrderPrintData | null> {
