...
import { QuotationPrintData } from './models/quotation-print-data.model';

@Injectable({
  providedIn: 'root'
})
export class QuotationPrintService {
  private printDataSubject = new BehaviorSubject<QuotationPrintData | null>(null);

  setPrintData(data: QuotationPrintData): void {
    this.printDataSubject.next(data);
  }

  getPrintData(): Observable<QuotationPrintData | null> {
